...
# setwd("../../data/last_run/postprocessing/")
...
```{r group_arguments, results = 'asis'}
group_names = data.frame(name = names(args)) %>% filter(str_detect(name, "^group"))
...
args %>% 
  ...
  cbind(group_names) %>%
  select(c(5,1,2,3,4)) %>%
  `colnames<-`(c('name','count','hashpower','latency(ms)','image')) %>%
  mutate(hashpower = as.numeric(as.character(hashpower))) %>%
  mutate(hashpower = scales::percent(hashpower)) %>%

  kable( format = 'latex'
       , caption ='Group arguments'
       , row.names = FALSE)  %>%
  kable_styling(latex_options = c('scale_down','HOLD_position'))
```
...
```{r fork_hist, fig.cap='Number of tips at height'}
hist(tips$height)
```
