...
pexec \
        ...
        -o - \
        --shell /bin/bash \
        -- /usr/bin/docker exec parallel_test_2  bash -c "target=$target; echo T \$target; now=\$(date +%s.%N); echo N \$now; curry=\$(echo \"\$target - \$now\" | bc); echo D \$curry; sleep \$curry; echo time=\$(echo \"\$now - \$target\" | bc)"
