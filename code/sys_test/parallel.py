...
class test_container():
    ...
    @property
    def conatiner_name(self):
        return self.conatiner_name

    def __init__(self):
        self.container_name="parallel_test"
        return None

    def __enter__(self):
        subprocess.check_output(f"docker run -dt --name={self.container_name} ubuntu top", shell=True, executable='/bin/sh')
        return self
    
    def __exit__(self ,type, value, traceback):
        subprocess.check_output(f"docker stop {self.container_name}", shell=True, executable='/bin/sh')
        subprocess.check_output(f"docker rm {self.container_name}", shell=True, executable='/bin/sh')

class test_containers():

    @property
    def conatiner_name_1(self):
        return self.conatiner_name_1

    @property
    def conatiner_name_2(self):
        return self.conatiner_name_2

    def __init__(self):
        self.container_name_1="parallel_test_1"
        self.container_name_2="parallel_test_2"
        return None

    def __enter__(self):
        subprocess.check_output(f"docker run -dt --name={self.container_name_1} ubuntu top", shell=True, executable='/bin/sh')
        subprocess.check_output(f"docker run -dt --name={self.container_name_2} ubuntu top", shell=True, executable='/bin/sh')
        return self
    
    def __exit__(self ,type, value, traceback):
        subprocess.check_output(f"docker stop {self.container_name_1}", shell=True, executable='/bin/sh')
        subprocess.check_output(f"docker stop {self.container_name_2}", shell=True, executable='/bin/sh')
        subprocess.check_output(f"docker rm {self.container_name_1}", shell=True, executable='/bin/sh')
        subprocess.check_output(f"docker rm {self.container_name_2}", shell=True, executable='/bin/sh')
...
def pexec():
    ...
    with test_container() as t:
        ...
        -o - \
        --shell /bin/bash \
        --shell-command '/usr/bin/docker exec {t.container_name} sh -c "sleep 1; date +%s.%N"' &
        """
